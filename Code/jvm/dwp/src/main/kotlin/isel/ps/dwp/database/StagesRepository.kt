...
class StagesRepository(private val handle: Handle) : StagesInterface {
    ...
    override fun signStage(stageId: String, approve: Boolean, userAuth: UserAuth) {
        ...
        val nextPendingStage = nextPendingStage(processId)
                ?: throw ExceptionControllerAdvice.InvalidParameterException("Este processo já terminou.")
    ...
    fun isUserInStage(stageId: String, userEmail: String): Boolean {
     ...
     fun isUserInProcessFromStageId(stageId: String, userEmail: String): Boolean {
        val sql = """SELECT CASE
WHEN COUNT(*) > 0 THEN true
ELSE false
END AS IsUserInProcessByStage from utilizador_etapa ue inner join etapa e on  e.id_processo = (select id_processo from etapa e inner join utilizador_etapa ue on ue.id_etapa = :etapaId limit 1) where email_utilizador = :userEmail ;""".trimIndent()

        // Execute the query, assuming handle is a Handle instance.
        val result =
                handle.createQuery(sql)
                        .bind("etapaId", stageId)
                        .bind("userEmail", userEmail)
                        .mapTo(Boolean::class.java)
                        .one()
